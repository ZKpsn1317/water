...
class RegionController extends BaseController
{
    ...
    {
        return [
            'region_name' => [ 'name' => '区域名称', 'value' => '', 'type' => 'text', 'searchType' => '=' ],
        ];
    }
    ...
    public function index()
    {
        ...
        if ( isset( $where['region_name'] ) && $where['region_name'] ) {
            $where['region_name'] = [ 'like', '%' . $where['region_name'] . '%' ];
        }

        $psize = 10;
        $page = input('page')?input('page'):1;
         if(input('export')) {
            $list = Region::where($where)->with('agent,device')->select();
            Excel::export($list, $this->exportField);
         } else {
            $list = Region::where($where)->with('agent,device')->page($page,$psize)->select();  //order('region_id DESC')->
         }
