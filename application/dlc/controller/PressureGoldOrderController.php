...
class PressureGoldOrderController extends BaseController
{
    ...
    public function index ()
    {
        ...
        if ( isset( $where['agent_id'] ) && $where['agent_id'] ) {
            $where['dlc_pressure_gold_order.agent_id'] = $where['agent_id'];
        ...
        if ( input( 'export' ) ) {
            $list = PressureGoldOrder::join( 'dlc_user', 'dlc_pressure_gold_order.user_id=dlc_user.user_id' )->order( 'pressure_gold_order_id DESC' )->where( $where )->with( 'user' )->select();
            Excel::export( $list, $this->exportField );
        } else {
            $list = PressureGoldOrder::join( 'dlc_user', 'dlc_pressure_gold_order.user_id=dlc_user.user_id' )->order( 'pressure_gold_order_id DESC' )->where( $where )->with( 'user' )->page( $page, $psize )->select();  //order('pressure_gold_order_id DESC')->
        }
