...
class UserCouponTypeController extends BaseController
{
    ...
    public function index()
    {
        ...
        $page = input('page')?input('page'):1;
         if(input('export')) {
            $list = UserCouponType::where($where)->select();
            Excel::export($list, $this->exportField);
         } else {
            $list = UserCouponType::where($where)->page($page,$psize)->select();  //order('UserCouponType_id DESC')->
         }
        
        
        $count = UserCouponType::where($where)->count();
    ...
    public function del()
    {
        ...
        $model = UserCouponType::get(['UserCouponType_id' => $id, 'agent_id' => $this->agent_id]);
