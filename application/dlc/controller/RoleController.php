...
class RoleController extends BaseController
{
	...
	public function authDetail(){
        ...
        $oath2 = model('roleOath')->where(array('module' => 'Dlc','parent_id' => array('NEQ',0), 'is_menu' => 1))-> order(array('order' => 'desc')) -> select();
    ...
    */
    public function authEdit(){
        ...
        $psize = 20;
        $cache = $m->alias('a')->join('dlc_role b ','a.roleid = b.role_id', 'left')->field('a.*,b.role_name')->where($map)->page($p, $psize)->select();
    ...
    public function roleDel()
    {
        ...
        if($id==1) {
            ...
            $model = \app\dlc\model\Role::get($id);
            if($model){
                $model->delete();
                
                /*删除后,这个角色后,角色下面的帐号都将失去权限*/
                $admins = \app\dlc\model\Admin::where('roleid', $model->role_id)->column('id');
