...
class OrderInfoController extends BaseController
{
    ...
    {
        $this->assign('shippingStatus', OrderInfo::$shippingStatusOption);
    }
    ...
    public function index()
    {
        ...
        $page = input('page')?input('page'):1;
         if(input('export')) {
            $list = OrderInfo::where($where)->with('order')->select();
            Excel::export($list, $this->exportField);
         } else {
            $list = OrderInfo::where($where)->with('order')->order('order_info_id DESC')->page($page,$psize)->select();  //order('order_info_id DESC')->
         }
        
        
        $count = OrderInfo::where($where)->count();
    ...
    public function add()
    {
        ...
        $this->assignOption();
        echo $this->fetch('order_info_form');
    }
