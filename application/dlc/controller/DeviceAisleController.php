...
class DeviceAisleController extends BaseController
{
    ...
    public function index()
    {
        ...
        $page = input('page')?input('page'):1;
         if(input('export')) {
            $list = DeviceAisle::where($where)->with('device')->select();
            Excel::export($list, $this->exportField);
         } else {
            $list = DeviceAisle::where($where)->with('device')->page($page,$psize)->select();  //order('device_aisle_id DESC')->
         }
        
        
        $count = DeviceAisle::where($where)->count();
    ...
    public function add()
    {
        ...
        $this->assignOption();
        echo $this->fetch('device_aisle_form');
    }
