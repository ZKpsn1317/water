...
class RewardLogController extends BaseController
{
    ...
    public function index()
    {
        ...
        $page = input('page')?input('page'):1;
         if(input('export')) {
            $list = RewardLog::where($where)->with('order,device,agent')->select();
            Excel::export($list, $this->exportField);
         } else {
            $list = RewardLog::where($where)->with('order,device,agent')->page($page,$psize)->select();  //order('id DESC')->
         }
    ...
    public function add()
    {
        ...
        $this->assignOption();
        echo $this->fetch('reward_log_form');
    }
