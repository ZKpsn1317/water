...
class BaseController extends Controller
{
    ...
    {
        //检测登陆状态
        if (!Session::has('admin_area_id')) {
            $this->redirect('area/Public/login');
        }
        $this->area_id = $area_id = Session::get('admin_area_id');
        $this->admin_id = $admin_id = Session::get('area_id');
        ...
        if(in_array($requestPath, ['index/index','index/main'])) {
            return true;
        }
    ...
    {
        $aothCacheName = \app\common\config\Confg::getAreaOathCacheName($userId);
