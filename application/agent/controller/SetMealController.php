...
class SetMealController extends BaseController
{
    ...
    public function index()
    {
        ...
        $page = input('page')?input('page'):1;
         if(input('export')) {
            $list = SetMeal::where($where)->select();
            Excel::export($list, $this->exportField);
         } else {
            $list = SetMeal::where($where)->page($page,$psize)->select();  //order('setmeal_id DESC')->
         }
        
        
        $count = SetMeal::where($where)->count();
    ...
    public function del()
    {
        ...
        $model = SetMeal::get(['setmeal_id' => $id, 'agent_id' => $this->agent_id]);
