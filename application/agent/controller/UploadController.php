...
class UploadController extends BaseController
{
    ...
    public function indeximg()
    {
        ...
        $page = '1,8';
        $m = Db::name('upload');
        $cache = $m->where(['agent_id' => $this->agent_id])->page($page)->order('id desc')->select();
    ...
    public function file()
    {
        ...
        if ($info) {
            echo json_encode(array('code'=>1, 'msg'=>'ok', 'data'=>  $this->request->domain() . '/public/uploads/imgs/'.date("Ymd") .'/'.$info->getFilename()));
        } else {
