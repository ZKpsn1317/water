...
class RoleController extends BaseController
{
	...
	public function authDetail(){
        ...
        $agent_id = \think\Session::get('agent.agentId');
        if(request()->isPost()) {
            $sel = I('post.sel')?:1;
            $inp = I('post.inp')?:1;
            $post=I('post.');
            ...
            if($data['oath_id'] = I('post.oath_id')) { //修改
                ...
                $a = (new RoleOathAgent())->save($data);
            } else {
                $a = (new RoleOathAgent())->add($data);
                $Dlc_auth = Db::name('role_agent')-> where(array('role_id' => 1))-> getField('auth');
                (new RoleAgent())->save(array('role_id' => 1,'auth' => $Dlc_auth.",".$a));
            }
            if($a) {
        ...
        $oath1 = Db::name('role_oath_agent')->where(array('module' => 'Dlc','is_menu' => 1,'parent_id' => 0))-> order(array('order' => 'desc')) -> select();
        $oath2 = Db::name('role_oath_agent')->where(array('module' => 'Dlc','parent_id' => array('NEQ',0), 'is_menu' => 1))-> order(array('order' => 'desc')) -> select();
    ...
    */
    public function roleList(){
        ...
        $agent_id = \think\Session::get('agent.agentId');
        $this->assign('breadhtml',$this->getBread($bread));
        $list = RoleAgent::where(['agent_id' => $agent_id])->select();
		$count = RoleAgent::where(['agent_id' => $agent_id])->count();
        $psize = 10;
	...
	public function roleDetail(){
        ...
        if(request()->isPost()) {
            ...
            if($role_id = input('post.role_id')) {
                $re = RoleAgent::where('role_id',$role_id)->update(array('role_name' => $role_name,'auth' => $oath));
            } else {
                $re = RoleAgent::insert(array('role_name' => $role_name,'auth' => $oath,'ctime' => time(), 'agent_id'=>$agent_id));
            }
            if($re !== false) {
                return(array('status' => 1,'msg' => '操作成功'));
            } else {
            ...
            if($role_id = input('role_id')) {
                $res = RoleAgent::where('role_id',$role_id)->find();
                if(!$res) return(array('status' => 0,'msg' => '该角色不存在'));
                $this->assign('res',$res);
            }
        }
        $oath1 = Db::name('role_oath_agent')->where(array('is_menu' => 1,'parent_id' => 0))->order(array('order' => 'desc'))->select();
        $oath2 = Db::name('role_oath_agent')->where(array('parent_id' => array('NEQ',0)))->order(array('is_menu' => 'desc','order' => 'desc'))->select();
        $this->assign('oath1', $oath1);
        $this->assign('oath2', $oath2);
    ...
    */
    public function authEdit(){
        ...
        $psize = 20;
        $cache = $m->alias('a')->join('dlc_role_agent b ','a.roleid = b.role_id', 'left')->field('a.*,b.role_name')->where($map)->page($p, $psize)->select();
        $count = $m->alias('a')->join('dlc_role_agent b ','a.roleid = b.role_id')->field('a.*,b.role_name')->where($map)->count();
    ...
    public function roleDel()
    {
        ...
        if($id==1) {
            ...
            $model = \app\agent\model\RoleAgent::get(['role_id'=>$id, 'agent_id'=>$agent_id]);
            if($model){
                $model->delete();

                /*删除后,这个角色后,角色下面的帐号都将失去权限*/
                $admins = \app\agent\model\AdminAgent::where('roleid', $model->role_id)->column('id');
